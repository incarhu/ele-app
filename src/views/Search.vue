<template>
	<div class="search">
		<Header :isLeft="true" title="搜索" @click.native="back" />
		...
		<div class="shop" v-if="result && !showShop">
			...
			<div v-else>	
				<SearchIndex :data="result.restaurants" @click="handleShop" />
		...
		<div class="container" v-if="showShop">
			...
			<div class="showlist" v-infinite-scroll="loadMore" :infinite-scroll-disabled="loading">
...
export default {
	...
	methods: {
		...
		keyWordChange() {
			if(this.keyWord != '') {
				this.$axios.get(`/api/profile/typeahead/${this.keyWord}`).then(res=> {
